...
async function startup() {
  try {
    ...
    // Step 10: Ensure default credentials for modules (Customer, Admin, Seller)
    try {
      const { ensureDefaultCredentials } = await import('../services/seedCredentials.js');
      await ensureDefaultCredentials();
    } catch (error) {
      console.error('[Startup] Warning: Failed to seed default credentials:', error.message);
    }
