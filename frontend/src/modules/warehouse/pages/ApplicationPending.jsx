...
const ApplicationPending = () => {
  ...
  if (!isLoading && isAuthenticated && role === "Warehouse") {
    const isApproved =
        ...
        >
          <div className="mb-8 flex items-center justify-between gap-4">
            <div className="inline-flex items-center gap-3 rounded-xl border border-white/15 bg-white/5 px-4 py-2">
              ...
              <span className="text-sm font-bold text-white/90">{appName} Warehouse</span>
            </div>
            <div
          ...
          <h1 className="text-3xl md:text-4xl font-black text-white leading-tight">
            {isRejected
              ? "Your Warehouse application needs action."
              : "Your Warehouse application is under review."}
          </h1>
          <p className="mt-4 text-base md:text-lg text-slate-200/90 font-medium max-w-2xl">
            {isRejected
              ? "You cannot access the Warehouse Dashboard yet. Please contact admin support and re-submit with the required details."
